...
normalize_archive() { archive=$1; destination=$2; weaken=${3:-no}; objects=$work/objects-$(basename "$destination"); mkdir -p "$objects"; (cd "$objects"; ar -x "$archive"; chmod 644 ./*.o; if [ "$weaken" = yes ] && [ -f compiler_rt.o ]; then objcopy=$(dirname "$(command -v zig)")/llvm-objcopy; [ -x "$objcopy" ] || objcopy=$(command -v llvm-objcopy); "$objcopy" --weaken-symbol=___isPlatformVersionAtLeast compiler_rt.o; fi; libtool -static -o "$destination" ./*.o); ranlib "$destination"; }
